...
public enum WordGroupSeperator: String, CaseIterable, Identifiable {
    case newLine = "\n"
    case space = " "
...
struct Kebab: TextStyle {
    ...
    init(components: [String]) { self.content = Self.join(components) }
}
...
struct Camel: TextStyle {
    static var style = WordGroupCase.camel
    static var separator = ""
    static var splitSeparator = WordCaseSplitSeparator.regex(["([A-Z]+)([A-Z][a-z]|[0-9])", "([a-z])([A-Z]|[0-9])", "([0-9])([A-Z])"])
    static var firstWordCase = WordCase.lowercase
    static var restWordCase = WordCase.capital
