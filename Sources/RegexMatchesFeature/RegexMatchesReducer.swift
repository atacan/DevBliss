...
public struct RegexMatchesReducer: ReducerProtocol {
    ...
    public struct State: Equatable {
        ...
        public init(inputOutput: InputAttributedTwoOutputAttributedEditorsReducer.State = .init(), regexPattern: String = "") {
            self.inputOutput = inputOutput
            self.regexPattern = regexPattern
        }
        
        public init(input: String, output: String = "") {
            self.inputOutput = .init(input: .init(text: .init(string: input)), output: .init(text: .init(string: output)))
    ...
    public var body: some ReducerProtocol<State, Action> {
        ...
        Reduce<State, Action> { state, action in
                ...
                return
                    .run { [input = state.inputOutput.input.text, regexPattern = state.regexPattern] send in
                        await send(
                            .conversionResponse(
                                TaskResult {
                                    let config = RegexMatchesConfig.init(wholeMatchColor: ThemeColor.Text.highlightedTextSecondary, capturedGroupColor: ThemeColor.Text.highlightedTextPrimary)
            ...
            case let .conversionResponse(.success(output)):
                ...
                _ = state.inputOutput.outputSecond.updateText(output.output.flatMap(\.capturedGroups).joined(separator: "\n"))
