    ...
    public static var liveValue: UrlEncodeClient {
        UrlEncodeClient(
            encode: { input, mode in
                switch mode {
                case .rfc3986:
                    // RFC3986 - stricter encoding, encodes more characters
                    var allowed = CharacterSet.urlQueryAllowed
                    allowed.remove(charactersIn: "!*'();:@&=+$,/?%#[]")
                    return input.addingPercentEncoding(withAllowedCharacters: allowed) ?? input

                case .formData:
                    // Form Data (application/x-www-form-urlencoded) - uses + for spaces
            ...
            looksEncoded: { input in
                // Check if string contains %XX patterns (percent-encoded characters)
